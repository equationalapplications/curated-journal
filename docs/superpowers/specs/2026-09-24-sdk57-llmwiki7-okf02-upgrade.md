...
- Guards, in order: `shouldContinue()` false (stop, `exhausted: false`); `remaining` not shrinking (`noProgress: true` — protects against non-convergent candidates); safety cap 200 batches (clamped to ≥ 1; `exhausted: true`).
- The summary `HealStepSummary = { batches, skipped, degraded, exhausted, noProgress, remaining }` is **surfaced, not discarded**: stored on machine context as `lastHealSummary` (reset on `START_NIGHT_SHIFT`), `exhausted`/`noProgress` stops logged via `console.warn`, and aggregate `skipped`/`degraded` counts available to the Night Shift summary UI. `skipped` shape changed `number → Array<{id, reason}>` — display code reads the aggregated count, not the raw array.
