...
- `src/lib/healLoop.ts` (pure, unit-tested):
  ...
  Guards, checked in order AFTER each batch (never before the first): `remaining === 0` → done (`exhausted: false`, takes priority over the cap); `!shouldContinue()` → stop (`exhausted: false`); `remaining` not shrinking vs previous iteration → stop (`noProgress: true`); `batches >= maxBatches` (default 200, clamped to ≥ 1) → stop (`exhausted: true`). `console.warn` on exhausted/noProgress, and on any stop where the abort signal fired (partial-summary trace for invoke-stop paths).
- `journalWikiMachine.ts`:
  ...
  - New context field `nightShiftSignal: { aborted: boolean }` — object constructed **in the context factory** (not caller-supplied, so a `[wiki]`-triggered actor rebuild can't inherit a stale signal); plus `lastHealSummary: HealStepSummary | null`.
  ...
  - `step.onDone`: `actions: assign({ lastHealSummary: ({ context, event }) => (event.output && typeof event.output === 'object' && 'batches' in event.output ? event.output : context.lastHealSummary) })` (writes on completion/explicit-abort only; invoke-stop paths are covered by the loop's partial-summary `console.warn`).
  - One-line pre-existing debt fixed while here: the `nightShift.IMPORT` transition also clears night-shift state (`queue: [], queueIndex: 0, aborted: false`) — otherwise after an import interrupts a heal, stale queue keeps `isBusyState` true and every later IMPORT detours through `busyRetry`.
  - `useJournalWiki.tsx`: **unchanged** — line 53 already forwards verbatim (`(entityId: string) => maintenanceRef.current.runHeal(entityId)`); only its inferred return type flows through. No edit.
...
**Files:** Create: `src/lib/healLoop.ts`, `__tests__/healLoop.test.ts`. Modify: `src/machines/journalWikiMachine.ts`, `__tests__/journalWikiMachine.test.ts`. (`src/hooks/useJournalWiki.tsx` unchanged.)

**Steps:**
0. From the installed 7.7.4 `.d.ts`: confirm (a) the exact `HealResult` shape, (b) the library's `runHeal` parameter list (verbatim forwarding must be correct), and (c) — critically — the hook's `runHeal` **return type**: if `useWikiMaintenance().runHeal` resolves `void` (status-in-hook-state pattern) instead of `HealResult`, use the pre-decided fallback: the machine calls `input.wiki.runHeal(entityId)` directly (wiki is already on context) and the provider adapter keeps returning whatever the hook returns for UI status. Record which variant was needed in the PR.
1. **Failing tests** (`__tests__/healLoop.test.ts`): drain 2→1→0 (accumulate skipped/degraded, `batches: 3`); `shouldContinue() === false` stops after current batch; no-progress stop (`noProgress: true`); cap (`maxBatches: 3` → `exhausted: true`, exactly 3 calls); `maxBatches: 0` clamps to 1 call; **cap reached but last batch returns `remaining: 0` → clean finish (`exhausted: false`)**; `shouldContinue` is NOT consulted before the first batch (a `shouldContinue` that starts `false` still runs batch 1). Loop termination order AFTER each batch: `remaining === 0` → done; else `!shouldContinue()` → stop; else no-progress → stop; else cap → stop.
2. **Failing machine tests** (extend `__tests__/journalWikiMachine.test.ts`):
   ...
   - IMPORT during multi-batch heal: assert the loop stops (call count stops growing), the import completes through `busyRetry`, and the post-import `idle` state has the night-shift queue cleared (guards both the orphaned-loop scenario and the pre-existing stale-queue `isBusyState` debt);
